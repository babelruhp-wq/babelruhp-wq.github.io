...
import '../../features/female_contracts_table/female_contracts_table_cubit.dart';
import '../../features/female_contracts_table/female_contracts_table_state.dart';

import 'female_contracts_table_widgets/female_contracts_table_appbar.dart';
...
class FemaleContractsTableScreen extends StatefulWidget {
  ...
  final Future<bool> Function(BuildContext context, FemaleContract contract)
      onEdit;

  const FemaleContractsTableScreen({
    super.key,
  ...
  @override
  State<FemaleContractsTableScreen> createState() =>
      _FemaleContractsTableScreenState();
}

class _FemaleContractsTableScreenState
    extends State<FemaleContractsTableScreen> {
  late final FemaleContractsTableCubit cubit;

  @override
  void initState() {
    super.initState();

    cubit = FemaleContractsTableCubit(
  ...
  Widget build(BuildContext context) {
    return BlocProvider<FemaleContractsTableCubit>.value(
      value: cubit,
      child: Scaffold(
        backgroundColor: const Color(0xffF5F7FA),
        appBar: FemaleContractsTableAppbar(
          title: widget.title,
          onBack: widget.onBack,
        ),
        body: BlocBuilder<FemaleContractsTableCubit, FemaleContractsTableState>(
          builder: (context, state) {
            final cubit = context.read<FemaleContractsTableCubit>();
            final token = context.watch<AuthProvider>().token ?? '';

            if (state.status == FemaleContractsTableStatus.loading) {
              return const Center(child: CircularProgressIndicator());
            }

            if (state.status == FemaleContractsTableStatus.error) {
            ...
            return Stack(
              children: [
                ...
                if (state.status == FemaleContractsTableStatus.deleting)
                  Container(
  ...
  Widget _buildViewByStatus(
    ...
    required FemaleContractsTableCubit cubit,
    required List<FemaleContract> items,
    required String token,
    required Future<void> Function() onRefresh,
    required void Function(FemaleContract c) onDetails,
    required Future<void> Function(FemaleContract c) onEdit,
  }) {
    ...
    if (f == 1) {
      return NewContractsView(
        ...
        /// لازم NewContractsView يكون معرف onCancel(FemaleContract, String reason)
        onCancel: (c, by, reason) async {
          if (token.isEmpty) return;
          await cubit.cancelContract(
            contractId: c.id,
            cancelationBy: by,
  ...
  void _openDetails(BuildContext context, FemaleContract c) {
    String status = '';
