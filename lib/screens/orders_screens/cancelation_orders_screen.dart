              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      tr(context, 'cancelation_orders_subtitle'),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: GoogleFonts.cairo(
                        fontSize: 13.5,
...
class CancelationOrdersScreen extends StatelessWidget {
  final double maxWidth;
  final VoidCallback? onBack;

  const CancelationOrdersScreen({
    super.key,
    this.maxWidth = 1400,
    this.onBack,
  });
      ...
      builder: (context, snap) {
        ...
        return BlocProvider(
          create: (_) => CancelationOrdersCubit(
            service: CancelationOrdersService(baseUrl: mainUrl),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: const [
                          _Header(),
                          SizedBox(height: 12),
                          Expanded(child: _CancelationOrdersTableLayout()),
                        ],
                      ),
                    ),
                  ),
  ...
  class _CancelationOrdersTableLayout extends StatefulWidget {
  const _CancelationOrdersTableLayout();

  @override
  State<_CancelationOrdersTableLayout> createState() =>
      _CancelationOrdersTableLayoutState();
}

class _CancelationOrdersTableLayoutState
    extends State<_CancelationOrdersTableLayout> {
  ...
  Widget build(BuildContext context) {
    return Theme(
      ...
      child: BlocBuilder<CancelationOrdersCubit, CancelationOrdersState>(
        builder: (context, state) {
          final cubit = context.read<CancelationOrdersCubit>();
          ...
          return Column(
            children: [
                    ...
                    Expanded(
                      child: SizedBox(
                        ...
                        child: TextField(
                          ...
                          onChanged: cubit.setQuery, // ✅ نفس الآلية
              ...
              // =========================
              // ✅ Pagination footer (نفس UnderProcess)
              // =========================
              Container(
                padding:
  ...
  static String _cancelByLabel(int v) {
    if (v == 1) return 'المكتب';
    if (v == 2) return 'العميل';
    return '—';
  }

  static Widget _statusChip(CancelationOrderStatus s) {
    String t;
    Color bg;
    Color fg;

    switch (s) {
      case CancelationOrderStatus.newOrder:
        t = 'طلب جديد';
        bg = const Color(0xFFFFF3E0);
        fg = const Color(0xFFB45309);
        break;
      case CancelationOrderStatus.approved:
        t = 'تم التأكيد';
        bg = const Color(0xFFE9F7EF);
        fg = const Color(0xFF1E7D43);
        break;
      case CancelationOrderStatus.rejected:
        t = 'مرفوض';
...
class _CancelationActions extends StatelessWidget {
  final CancelationOrdersRow row;
  const _CancelationActions({required this.row});
    ...
    return Wrap(
      ...
      children: [
        _pill(
          ...
          onTap: row.status == CancelationOrderStatus.approved
              ? null
              : () async {
            final ok = await _confirmDialog(
              ...
              message: 'هل أنت متأكد من الموافقة على الإلغاء؟',
            );
            if (ok == true) {
              await cubit.approveOrder(row); // ✅ نفس الآلية
            }
          },
        ),
        _pill(
          ...
          onTap: row.status == CancelationOrderStatus.rejected
              ? null
              : () async {
            final ok = await _confirmDialog(
              context,
              ...
              title: 'رفض الإلغاء',
              message: 'هل أنت متأكد من رفض الإلغاء؟',
            );
            if (ok == true) {
              await cubit.rejectOrder(row); // ✅ نفس الآلية
            }
          },
        ),
      ],
