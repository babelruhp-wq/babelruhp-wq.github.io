...
class AddFemaleContractFormController {
  // =========================
  // Controllers
  // =========================
  ...
  String? statusValue; // '1'..'5'
  String? experienceValue; // 'true'/'false'

  /// ✅ جديد: نوع العقد (1 = معين باسم, 2 = حسب المواصفات)
  ...
  // =========================
  // Internal helpers for auto status
  // =========================
  String? _statusBeforeApproved;
  String? _statusBeforeArrival;
  ...
  bool contractHasErrors(BuildContext context) {
    ...
    if (dropdownRequired(visaTypeValue, tr(context, 'choose_visa_type')) != null) {
      return true;
    }

    // ✅ جديد: نوع العقد إجباري
    if (dropdownRequired(contractTypeValue, tr(context, 'choose_contract_type')) != null) {
      return true;
    }

    if (dropdownRequired(statusValue ?? '1', tr(context, 'choose_contract_status')) != null) {
      return true;
    }
  ...
  /// ✅ autoStatus=false في prefill عشان ما يغيرش الحالة اللي جاية من السيرفر
  void setArrivalDate(DateTime? d, {bool autoStatus = true}) {
    arrivalDate = d;
    ...
    if (d != null) {
      _statusBeforeArrival ??= statusValue;
      statusValue = '4'; // الوصول
    } else {
      statusValue = _statusBeforeArrival ?? statusValue;
      _statusBeforeArrival = null;
    }
  }
  ...
  // Approved behavior
  // =========================
  /// ✅ autoStatus=true فقط لما المستخدم يغيّر، مش في prefill
  void setApproved(bool v, {bool autoStatus = true}) {
    ...
    if (approved) {
      if (autoStatus) {
        _statusBeforeApproved ??= statusValue;
        statusValue = '2'; // تحت الإجراء
      }
    } else {
      approvalDate = null;
      ...
      if (autoStatus) {
        statusValue = _statusBeforeApproved ?? statusValue;
        _statusBeforeApproved = null;
      }
    }
  }
  ...
  // Prefill (Edit)
  // =========================
  void prefillFromContract(FemaleContract c) {
    ...
    // ✅ جديد: لازم يكون موجود في الموديل
    // لو اسم الحقل عندك مختلف عدله هنا:
    contractTypeValue = (c.contractType ?? 0).toString(); // 1 or 2

    statusValue = c.contractStatus.toString();
    ...
    sponsorAltPhoneCtrl.text = _extract8DigitsFromKsa(c.sponsorSecondaryPhoneNumber ?? '');
    ...
    if ((c.arrivalDate ?? '').trim().isNotEmpty) {
      setArrivalDate(DateTime.tryParse(c.arrivalDate!), autoStatus: false);
    } else {
      setArrivalDate(null, autoStatus: false);
    }
  ...
  Map<String, dynamic> buildPayload({
    ...
    final payload = <String, dynamic>{
      ...
      // ✅ جديد: نوع العقد (1/2)
      "contractType": int.parse(contractTypeValue!), // <-- لو اسم الحقل مختلف عدله هنا
      ...
      "contractStatus": int.parse(statusValue ?? '1'),
      "approved": approved,
