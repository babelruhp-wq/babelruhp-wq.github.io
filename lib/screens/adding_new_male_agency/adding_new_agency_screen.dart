...
import '../../features/add_new_agency/add_male_agency_cubit.dart';
import '../../features/add_new_agency/add_male_agency_state.dart';
...
import 'adding_new_contract_screen/add_male_agency_form_controller.dart';

/// ✅ Call this بدل Navigator.push
Future<bool?> showAddMaleAgencyDialog(
    BuildContext context, {
      required String officeId,
      required String countryName,
      required String officeName,
      required String baseUrl,
      MaleAgency? initialAgency, // null => add
    }) async {
...
class AddMaleAgencyDialogBody extends StatefulWidget {
  final String officeId;
  final String countryName;
  final String officeName;
  final String baseUrl;
  final MaleAgency? initialAgency;

  /// true=changed, false=cancel
  final ValueChanged<bool> onClose;

  const AddMaleAgencyDialogBody({
    super.key,
  ...
  @override
  State<AddMaleAgencyDialogBody> createState() =>
      _AddMaleAgencyDialogBodyState();
}

class _AddMaleAgencyDialogBodyState extends State<AddMaleAgencyDialogBody> {
  ...
  late final AddMaleAgencyFormController form;

  bool get isEdit => widget.initialAgency != null;
  ...
  void initState() {
    super.initState();
    form = AddMaleAgencyFormController();

    final a = widget.initialAgency;
    if (a != null) {
      form.prefillFromAgency(a);
    }
  }
  ...
  Future<void> _submit(AddMaleAgencyCubit cubit) async {
    ...
    if (!formOk ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(tr(context, 'please_fix_errors'))),
      );
      return;
    }
    ...
    if (isEdit) {
      final id = widget.initialAgency!.id.trim();
      if (id.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      await cubit.update(agencyId: id, payload: payload);
    } else {
  ...
  Widget build(BuildContext context) {
    final title = isEdit ? tr(context, 'edit') : tr(context, 'add_new_contract');

    return BlocProvider(
      create: (ctx) => AddMaleAgencyCubit(
        service: MaleAgenciesService(baseUrl: widget.baseUrl),
        auth: ctx.read<AuthProvider>(), // ✅ ctx مش context
      ),
      child: BlocConsumer<AddMaleAgencyCubit, AddMaleAgencyState>(
        listener: (context, state) {
          if (state.status == AddMaleAgencyStatus.success) {
          ...
          if (state.status == AddMaleAgencyStatus.error) {
            ScaffoldMessenger.of(context).clearSnackBars();
            ScaffoldMessenger.of(context).showSnackBar(
        ...
        builder: (context, state) {
          final cubit = context.read<AddMaleAgencyCubit>();
          final isLoading = state.status == AddMaleAgencyStatus.loading;
                                ...
                                child: Form(
                                  ...
                                  child: Column(
                                    children: [
                                          ...
                                          TwoColumns(
                                            children: [
                                              ...
                                              AppFormField(
                                                ...
                                                validator: (v) =>
                                                    form.ageValidator(context, v),
                                              ),
                                              AppFormField(
                                                label: tr(context, 'salary'),
                                                hint: tr(context, 'enter_salary'),
                                                ...
                                                validator: (v) =>
                                                    form.salaryValidator(context, v),
                                              ),
                                              AppDropdown(
                                                ...
                                                items: [
                                                  DropdownMenuItem(
                                                    value: 'true',
                                                    child: Text(tr(context, 'experience_yes')),
                                                  ),
                                                  DropdownMenuItem(
                                                    value: 'false',
                                                    child: Text(tr(context, 'experience_no')),
                                                  ),
                                                ],
                                                ...
                                                validator: (v) =>
                                                    form.dropdownRequired(
                                                      v,
                                                      tr(context, 'choose_experience_value'),
                                                    ),
                                              ),
                                              AppDropdown(
                                                ...
                                                items: [
                                                  DropdownMenuItem(
                                                    value: '1',
                                                    child: Text(tr(context, 'religion_islam')),
                                                  ),
                                                  DropdownMenuItem(
                                                    value: '2',
                                                    child: Text(tr(context, 'religion_christian')),
                                                  ),
                                                  DropdownMenuItem(
                                                    value: '3',
                                                    child: Text(tr(context, 'religion_other')),
                                                  ),
                                                ],
                                                onChanged: (v) => setState(
                                      ...
                                      // Contract + Visa
                                      ContractSection(
                                        title: tr(context, 'contract_visa_details'),
                                            ...
                                            children: [
                                              AppDropdown(
                                                ...
                                                items: [
                                                  DropdownMenuItem(
                                                    value: '1',
                                                    child: Text(tr(context, 'contract_type_named')),
                                                  ),
                                                  DropdownMenuItem(
                                                    value: '2',
                                                    child: Text(tr(context, 'contract_type_specs')),
                                                  ),
                                                ],
                                                onChanged: (v) => setState(
                                                      () => form.contractTypeValue = v,
                                                ),
                                              ...
                                              AppDropdown(
                                                ...
                                                items: [
                                                  DropdownMenuItem(
                                                    value: '1',
                                                    child: Text(tr(context, 'visa_type_full')),
                                                  ),
                                                  DropdownMenuItem(
                                                    value: '2',
                                                    child: Text(tr(context, 'visa_type_normal')),
                                                  ),
                                                ],
                                                onChanged: (v) =>
                                                    setState(() => form.visaTypeValue = v),
                                                validator: (v) =>
                                                    form.dropdownRequired(
                                                      v,
                                                      tr(context, 'choose_visa_type'),
                                                    ),
                                              ),
                                              AppFormField(
                                                label: tr(context, 'visa_number'),
                                                hint: tr(context, 'visa_number_hint'),
                                                icon: Icons.credit_card,
                                                controller: form.visaNumberCtrl,
                                                validator: (v) =>
                                                    form.visaNoValidator(context, v),
                                              ),
                                              AppFormField(
                                                label: tr(context, 'contract_number'),
                                                hint: tr(context, 'contract_number_hint'),
                                                icon: Icons.article_outlined,
                                                controller: form.contractNumberCtrl,
                                                validator: (v) =>
                                                    form.contractNoValidator(context, v),
                                              ),
                                              DateField(
                                                label: tr(context, 'contract_date'),
                                                icon: Icons.date_range,
                                                value: form.contractDate,
                                                controller: form.contractDateCtrl,
                                                required: true,
                                                onPick: () => _pickDate(
                                      ...
                                      ContractSection(
                                        ...
                                        children: [
                                          TwoColumns(
                                            children: [
                                              AppFormField(
                                                label: tr(context, "sponsor_name"),
                                                hint: tr(context, "enter_sponsor_name"),
                                                icon: Icons.person_search,
                                                controller: form.sponsorNameCtrl,
                                                validator: (v) =>
                                                    form.requiredValidator(
                                                      v,
                                                      tr(context, "enter_sponsor_name"),
                                                    ),
                                              ),
                                              PhoneField(
                                                label: tr(context, "phone"),
                                                controller: form.sponsorPhoneCtrl,
                                                validator: (v) =>
                                              ...
                                              PhoneField(
                                                label: tr(context, "alternative_phone"),
                                                controller: form.sponsorAltPhoneCtrl,
                                                validator: (v) =>
                                                    form.phone8Validator(
                                                      context,
                                                      v,
                                              ...
                                              AppFormField(
                                                ...
                                                hint: tr(context, "enter_id_number"),
                                      ...
                                      ActionButtons(
                                        ...
                                        onSave: () async {
                                          ...
                                          // ✅ Confirm قبل الإرسال
                                          final ok = await _showConfirmSaveDialog(context);
                                          if (!ok) return;
